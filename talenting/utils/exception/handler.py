...
def custom_exception_handler(exc, context):
    ...
    if response is not None:
        ...
        if hasattr(view, 'get_fields_info') and (isfunction(getattr(view, 'get_fields_info')) or \
                ismethod(getattr(view, 'get_fields_info'))):
            model_name, fields = view.get_fields_info()
            if view_name in custom_view_exception_handlers:
                response.data = custom_view_exception_handlers[view_name](fields, model_name, error_response,
                                                                          status_code)
            else:
